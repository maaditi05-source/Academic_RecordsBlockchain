...
class StudentController {
    ...
    static async createStudent(req, res) {
        ...
        try {
            ...
            // Enroll student's own Fabric identity into the wallet so they can
            // authenticate with their own X.509 certificate (not fall back to admin).
            try {
                const caClient = FabricCAClient.getCAClientForRole('student');
                await caClient.registerUser(
                    rollNumber,   // wallet key = rollNumber (same as username after createStudentUser)
                    { role: 'student', department, email },
                    'client',
                    ''
                );
                logger.info(`Fabric wallet identity enrolled for student: ${rollNumber}`);
            } catch (enrollErr) {
                // Non-fatal: student can still log in via admin fallback
                logger.warn(`Could not enroll Fabric wallet identity for student ${rollNumber}: ${enrollErr.message}`);
            }
