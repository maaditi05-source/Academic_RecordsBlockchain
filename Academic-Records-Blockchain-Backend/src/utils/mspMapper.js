...
const getFabricIdentity = (user) => {
    ...
    // For students: wallet key = rollNumber = username (set by createStudentFromBlockchain.js).
    // Their wallet identity is enrolled under rollNumber, not DB user.id.
    if (user.role === 'student') {
        return user.username || user.userId || user.id;
    }

    // For department/verifier: wallet key = user.id (the DB id used in auth register flow).
    // These are enrolled during register() with user.id as the wallet key.
